...
public class AlphaOSpeed implements OthelloAI {
	private int player;
	private Block[][] board;
	private int cnt;
	private int lastCnt;
	...
	//private Integer max;
	public AlphaOSpeed(int player, Block[][] board, int cnt, int lastCnt, int level) {
		this.player = player;
		this.board = board;
		this.cnt = cnt;
	...
	private int routeSearchABP2(int currPlayer, Block b, Block[][] copyBoard, List<Block> emptyBlockList, int cnt,
		...
		for (int i = 0; i < impBlockList.size(); i++) {
			...
			int tempWinCnt = routeSearchABP2( nextPlayer, impBlockList.get(i),copyBoard , cloneEmptyBlockList, cnt + 1,
					!searchMax, min, max, winProb);
		...
		if ( cnt - this.cnt == 1 && this.cnt + level < lastCnt) {
			...
			if (currPos % scale == 0 || currPos % scale == 7 || currPos < scale
					|| currPos >= (scale * scale- scale)) {
				if(b.isDanger(this.player,scale)){
					System.out.println("# 놓을 수 : "+b.getStrSave()+" , 원래 알파 : "+max+" 마이너스 : "+(((lastCnt - cnt) / scale) + 1));
					ab -= ((lastCnt - cnt) / scale) + 1;
				}else{
					ab += ((lastCnt - cnt) / scale) + 1;
				}
