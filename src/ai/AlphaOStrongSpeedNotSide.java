...
public class AlphaOStrongSpeedNotSide implements OthelloAI {
	private int player;
	private Block[][] board;
	private int cnt;
	private int lastCnt;
	...
	// private Integer max;
	public AlphaOStrongSpeedNotSide(int player, Block[][] board, int cnt, int lastCnt, int level, GameView gameView) {
	...
	class MultiThinking extends Thread {
		...
		public void run() {
			...
			if (!isPerfectLearnning) {
				...
				if(choiceBlock.isSide && !choiceBlock.isCorner){
					gameView.appendText("Need Not Side Block !("+choiceBlock.getStrSave()+")");
					this.alpha -= 5;
				}
			}
