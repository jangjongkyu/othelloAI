...
public class AlphaOStrongSpeedSum implements OthelloAI {
	private int player;
	private Block[][] board;
	private int cnt;
	private int lastCnt;
	...
	// private Integer max;
	public AlphaOStrongSpeedSum(int player, Block[][] board, int cnt, int lastCnt, int level, GameView gameView) {
	...
	private Block think(List<Block> emptyBlockList) {
		...
		for (MultiThinking m : thinkingList) {
			...
			if (maxScore == winCnt) {
				...
				int currBlockRoute = m.getRouteNum();
				gameView.appendText("originRouteNum : "+minRouteNum+" , currRouteNum("+currBlock.getStrSave()+") : "+currBlockRoute);
				if(currBlockRoute < minRouteNum || (currBlockRoute == minRouteNum && r.nextBoolean())){
	...
	private int routeSearchABP2(int currPlayer, Block b, Block[][] copyBoard, List<Block> emptyBlockList, int cnt,
			boolean searchMax, Integer max, Integer min, WinProb winProb,int playerEat,int enemyEat) {
		// TODO ABP1
		...
		if (cnt > deps || cnt > lastCnt) {
			...
			if (isPerfectLearnning) {
				...
				res = calculateScoreOpenning(cnt, copyBoard, cloneEmptyBlockList,playerEat,enemyEat);
			}
			b.backUpLoad();
			return res;
		}
		...
		if(nextPlayer == player){
			playerEat += impBlockList.size();
		}else{
			enemyEat += impBlockList.size();
		}
		...
		for (int i = 0; i < forSize; i++) {
			...
			int tempWinCnt = routeSearchABP2(nextPlayer, nextChoice, copyBoard, cloneEmptyBlockList, cnt + 1,
					!searchMax, max, min, winProb, playerEat, enemyEat);
	...
	private int calculateScoreOpenning(int currCnt, Block[][] copyBoard, List<Block> emptyBlockList,int playerEat,int enemyEat) {
		// TODO:calculateScore
		calculCnt++;
		int aiImpossible = playerEat;
		int enemyImpossible = enemyEat;
