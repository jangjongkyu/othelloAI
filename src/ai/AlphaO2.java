...
public class AlphaO2 implements OthelloAI {
	private int player;
	private Block[][] board;
	private int cnt;
	private int lastCnt;
	...
	class MultiThinking extends Thread{
		...
		public void run() {
			...
			this.alpha = routeSearchABP(player, choiceBlock, copyBoard , emptyBlockList, cnt+1, false, null, null);
	...
	public AlphaO2(int player, Block[][] board, int cnt, int lastCnt, int level) {
		this.player = player;
		this.board = board;
		this.cnt = cnt;
	...
	private int routeSearchABP(int currPlayer, Block b, Block[][] copyBoard, List<Block> emptyBlockList, int cnt,
		...
		if (level > 5 /*&& cnt - this.cnt == 1 && this.player != currPlayer*/
				...
				if (num.equals("0") || num.equals(ne) || num.equals(sw) || num.equals(se)) {
					if(this.player == currPlayer){
						plusValue = ((((board.length * board.length - 4) - cnt) / (board.length / 2)) + 1);
					}else{
						plusValue = -((((board.length * board.length - 4) - cnt) / (board.length / 2)) + 1);
					}
				}
		...
		if (level > 5 && cnt - this.cnt == 1 && this.player != currPlayer
				...
				if(this.cnt + level < (scale * scale - 4)){
					for(Block nextBlock : impBlockList){
						int nextPos = Integer.parseInt(nextBlock.getStrSave());
						if (nextPos % scale == 0 || nextPos % scale == 7 || nextPos < scale
								|| nextPos >= (scale * scale- scale)) {
							System.out.println("# 놓을 수 : "+b.getStrSave()+" , 원래 알파 : "+ab+" 마이너스 : "+((((scale * scale - 4) - cnt) / scale) + 1)+" , 먹힐곳 : "+nextPos);
							int plusSide = 0;
