...
public class AlphaOStrongSpeed2 implements OthelloAI {
	private int player;
	private Block[][] board;
	private int cnt;
	private int lastCnt;
	...
	// private Integer max;
	public AlphaOStrongSpeed2(int player, Block[][] board, int cnt, int lastCnt, int level, GameView gameView) {
	...
	private Block think(List<Block> emptyBlockList) {
		...
		gameView.appendText(" [ 수읽기중.. ] 현재상태 : "+state);
		for (MultiThinking m : thinkingList) {
			// cnt++;
			try {
				m.join();
	...
	private int routeSearchABP2(int currPlayer, Block b, Block[][] copyBoard, List<Block> emptyBlockList, int cnt,
		...
		if (cnt > deps || cnt > lastCnt) {
			...
			if (isPerfectLearnning) {
				...
				if(isSecondEnding){
					res = calculateScoreSecondEnding(cnt, copyBoard, cloneEmptyBlockList);
				}else{
					res = calculateScoreOpenning(cnt, copyBoard);
				}
			}
			b.backUpLoad();
			return res;
	...
	private int calculateScoreOpenning(int currCnt, Block[][] copyBoard) {
		// TODO:calculateScore
				calculCnt++;
				int aiArea = 0;
				int enemyArea = 0;
	...
	private int calculateScoreSecondEnding(int currCnt, Block[][] copyBoard, List<Block> emptyBlockList) {
		// TODO:calculateScore
		calculCnt++;
		int aiImpossible = 0;
